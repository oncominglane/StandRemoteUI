...
class Telemetry:
    ...
    def _dispatch(self, data: dict):
        ...
        - CAN кадры (direction + id) → _handle_can_frame
        ...
        try:
            ...
            keys = set(data.keys())
            if keys & {"ns", "Ms", "Idc", "Isd", "Ud", "Uq", "Id", "Iq", "Emf", "Welectrical", "Wmechanical"}:
    ...
    def _handle_can_frame(self, data: dict):
        ...
        _set(11, data.get("ts", datetime.now().strftime("%H:%M:%S.%f")[:-3]))

        self.ui_log(f"[CAN {direction.upper()}] id={line[0].get()} data=" +
    ...
    def _handle_model_data(self, d: dict):
        ...
        if Wm is None and We is not None and self._last_pole_pairs:
            # ω_m = ω_e / p
            try:
                Wm = We / float(self._last_pole_pairs)
            except Exception:
                pass
        ...
        if isinstance(ev, dict):
            ...
            put("Ud", Ud); put("Uq", Uq); put("Id", Id); put("Iq", Iq)
        ...
        self._append_log_row({
            ...
            "Emf": Emf, "Welectrical": We, "motorRs": Rs, "Wmechanical": Wm,
        })
        ...
        # --- расчёт Ld/Lq (приближённо, устойчиво к нулям) ---
        # Модель PMSM (упрощ.): v_d = R_s i_d - ω L_q i_q
        #                       v_q = R_s i_q + ω L_d i_d + ω ψ_f  (ψ_f ≈ Emf/ω)
        Ld = None
        Lq = None
        if We and Id and Uq is not None and Iq is not None:
            psi_f = (Emf / We) if (Emf is not None and We) else 0.0
            try:
                # Ld из уравнения v_q
                Ld = (Uq - (Rs or self._last_rs) * (Iq or 0.0) - (We * psi_f)) / (We * Id)
            except Exception:
                Ld = None
        if We and Iq and Ud is not None and Id is not None:
            try:
                # Lq из уравнения v_d
                Lq = ((Rs or self._last_rs) * (Id or 0.0) - Ud) / (We * Iq)
            except Exception:
                Lq = None
        ...
        if rpm is not None:
            ...
            p_elec = None
            if Ud is not None and Id is not None and Uq is not None and Iq is not None:
    ...
    def _tick_trends(self):
        try:
            ...
            canvas = getattr(self.views, "trends_canvas", None)
            if canvas is None:
                # попробуем достать из matplotlib FigureCanvasTkAgg (как в простом view)
                # обычно хранится как FigureCanvasTkAgg(...).get_tk_widget().master
                pass

            # ожидаем порядок линий: (l_ns,l_ms,l_idc,l_isd,l_id,l_iq,l_ud,l_uq)
            # и 4 оси: ax1..ax4
            if axes and lines and len(lines) >= 8:
                x = list(range(len(self.state.trend_ns)))
                lines[0].set_data(x, list(self.state.trend_ns))
                lines[1].set_data(x, list(self.state.trend_Ms))

                x3 = list(range(len(self.state.trend_Idc)))
                lines[2].set_data(x3, list(self.state.trend_Idc))
                lines[3].set_data(x3, list(self.state.trend_Isd))

                x4 = list(range(len(self.state.trend_Id)))
                lines[4].set_data(x4, list(self.state.trend_Id))
                lines[5].set_data(x4, list(self.state.trend_Iq))
                lines[6].set_data(x4, list(self.state.trend_Ud))
                lines[7].set_data(x4, list(self.state.trend_Uq))
        ...
        try:
            ...
            if ln_torque:
                ...
                try:
                    ...
                    ax6 = mp.get("ax6"); ax6r = mp.get("ax6r") or mp.get("ax6_right")
                    for ax in (ax6, ax6r):
                        if ax:
                            ax.relim(); ax.autoscale_view()
                except Exception:
